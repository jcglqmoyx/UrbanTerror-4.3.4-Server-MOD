...
char *weapons_list[16] = {"hk69", "lr", "spas12", "m4", "psg1", "mp5k", "ak103", "negev", "g36", "sr8", "ump45",
                          "mac11", "p90", "frf1", "benelli", "fstod"};
char *items[3] = {"silencer", "laser",
                  "medkit"}; // Removed helmet & vest as now given always for a throwing knife kill.
...
qboolean wasKillNoscope(client_t *klr) {
    ...
    if (klr->lastUsercmd.buttons == 67585 || klr->lastUsercmd.buttons == 2081 || klr->lastUsercmd.buttons == 2049 ||
        klr->lastUsercmd.buttons == 67841 || klr->lastUsercmd.buttons == 2305) {
        noscope = qtrue;
    }
    return noscope;
}
        ...
        if (Q_stricmp(killer, "1022") != 0) {
            ...
            if (Q_stricmp(killer, killed) != 0) { // no suicide
                ...
                if (!Q_stricmp(wpn, "12:")) {
                    ...
                    SV_SendServerCommand(klr,
                                         "cchat \"\" \"^6~^8> ^1Throwing Knife ^7kill ^1= ^6%s ^3and ^2Vest+Helmet\"",
                                         SV_NameItem(item));
                    ...
                    Cmd_ExecuteString(va("gi %i helmet", ps->clientNum));
                    SV_SendServerCommand(klr,
                                         "cchat \"\" \"^6~^8> ^6Curb Stomp ^7kill ^1= ^5HK69/SPAS12/Smokes ^3and ^6all Items\"");
                    Cmd_ExecuteString(
                            va("bigtext \"%s made a ^6Curb Stomp^7!!\n^3He won ^5HK69/SPAS12/Smokes ^3and ^6all Items\"",
                               klr->name));
...
void parseClientSettings(client_t *client, char *message) {
    ...
    if (strcmp(Cmd_Argv(0), "!preference") == 0) {
        ...
        if (strcmp(type, "primary") != 0 && strcmp(type, "secondary") != 0 && strcmp(type, "pistol") != 0 &&
            ...
            SV_SendServerCommand(client,
                                 "cchat \"\" \"^5[^3Arena^5] ^1Error: [Unknown Type] ^3See ^2!guns ^3for a list of types.\"");
...
void SV_onChat(char *playerid, char *playername, char *message) {
    ...
    if (mod_gunsmod->integer) {
        if (strcmp(message, "!kitlist") == 0) {
            ...
            SV_SendServerCommand(cl,
                                 "print  \"^3////////////////////////////////////////////////////////////////////////////\n\"");
            SV_SendServerCommand(cl,
                                 "print  \"^3//               ^1! BUYING A KIT WILL LOSE ALL YOUR WEAPONS !              ^3//\n\"");
            SV_SendServerCommand(cl,
                                 "print  \"^3//-----------------------------------^3/------------------------------------^3//\n\"");
            SV_SendServerCommand(cl,
                                 "print  \"^7////////////////////////////////////////////////////////////////////////////\n\"");
            SV_SendServerCommand(cl,
                                 "print  \"^7//               Armor               ^7/             Attachments            ^7//\n\"");
            SV_SendServerCommand(cl,
                                 "print  \"^7//-----------------------------------^7/------------------------------------^7//\n\"");
            SV_SendServerCommand(cl,
            ...
            SV_SendServerCommand(cl,
                                 "print  \"^7//  ^5Usage:                                                                ^7//\n\"");
            SV_SendServerCommand(cl,
                                 "print  \"^7//  !kit <KIT SHORT/NAME>  for example: !kit armor / !kit ar              ^7//\n\"");
            SV_SendServerCommand(cl,
                                 "print  \"^7//                                      !kit attachments / !kit att       ^7//\n\"");
            SV_SendServerCommand(cl,
                                 "print  \"^7////////////////////////////////////////////////////////////////////////////\n\"");
            ...
            SV_SendServerCommand(cl,
                                 "print  \"^7////////////////////////////////////////////////////////////////////////////\n\"");
            SV_SendServerCommand(cl,
                                 "print  \"^7//           Sniper Rifles           ^7/         Assault Rifles             ^7//\n\"");
            SV_SendServerCommand(cl,
                                 "print  \"^7//-----------------------------------^7/------------------------------------^7//\n\"");
            SV_SendServerCommand(cl,
            ...
            SV_SendServerCommand(cl,
                                 "print  \"^7//........................................................................^7//\n\"");
            SV_SendServerCommand(cl,
                                 "print  \"^7//           Machine Guns            ^7/             Specials               ^7//\n\"");
            SV_SendServerCommand(cl,
                                 "print  \"^7//-----------------------------------^7/------------------------------------^7//\n\"");
            SV_SendServerCommand(cl,
            ...
            SV_SendServerCommand(cl,
                                 "print  \"^7//........................................................................^7//\n\"");
            SV_SendServerCommand(cl,
                                 "print  \"^7//             Abilites              ^7/             Equipment              ^7//\n\"");
            SV_SendServerCommand(cl,
                                 "print  \"^7//-----------------------------------^7/------------------------------------^7//\n\"");
            SV_SendServerCommand(cl,
            ...
            SV_SendServerCommand(cl,
                                 "print  \"^7// ^3!vampire <amount> 1 minute = 10000 coins                               ^7//\n\"");
            SV_SendServerCommand(cl,
                                 "print  \"^7//........................................................................^7//\n\"");
            SV_SendServerCommand(cl,
                                 "print  \"^7//  ^5Specials:                                                             ^7//\n\"");
            SV_SendServerCommand(cl,
                                 "print  \"^7//  !gamble <amount> (Gamble for a 50/50 chance to tripple your input!)   ^7//\n\"");
            SV_SendServerCommand(cl,
                                 "print  \"^7//  !particles (Will activate particles around you)^2Price:^610000000 coins^7//\n\"");
            SV_SendServerCommand(cl,
            ...
            SV_SendServerCommand(cl,
                                 "print  \"^7//  ^5Usage:                                                                ^7//\n\"");
            SV_SendServerCommand(cl,
                                 "print  \"^7//  !buy <WEAPON NAME>     for example: !buy sr8                          ^7//\n\"");
            SV_SendServerCommand(cl,
                                 "print  \"^7//  !buy <ITEM NAME>       for example: !buy vest                         ^7//\n\"");
            SV_SendServerCommand(cl,
                                 "print  \"^7//  ^5Autobuy:                                                              ^7//\n\"");
            SV_SendServerCommand(cl,
                                 "print  \"^7//  !buy <WEAPON NAME> ON  for example: !buy sr8 on                       ^7//\n\"");
            SV_SendServerCommand(cl,
                                 "print  \"^7//  !buy <ITEM NAME> ON    for example: !buy vest on                      ^7//\n\"");
            SV_SendServerCommand(cl,
                                 "print  \"^7//  ^5Abilites:                                                             ^7//\n\"");
            SV_SendServerCommand(cl,
                                 "print  \"^7//  !maptp (no !b required, simply use !<ability name>)                   ^7//\n\"");
            SV_SendServerCommand(cl,
                                 "print  \"^7//  !heal                                                                 ^7//\n\"");
            SV_SendServerCommand(cl,
                                 "print  \"^7////////////////////////////////////////////////////////////////////////////\n\"");
    ...
    if (mod_1v1arena->integer) {
        if (strcmp(message, "!guns") == 0) {
            ...
            SV_SendServerCommand(cl,
                                 "print  \"^7//  ^5Types:                                                                ^7//\n\"");
            SV_SendServerCommand(cl,
                                 "print  \"^7//  ^4primary / secondary / pistol / sniper / knife / gib                   ^7//\n\"");
            SV_SendServerCommand(cl,
                                 "print  \"^7//                                                                        ^7//\n\"");
            SV_SendServerCommand(cl,
                                 "print  \"^7//  ^5Weapons:                                                              ^7//\n\"");
    ...
    if (mod_gunsmod->integer || mod_levelsystem->integer) {
        ...
        if ((strcmp(message, "status") == 0) || (strcmp(message, "whats up") == 0) ||
            (strcmp(message, "status?") == 0) || (strcmp(message, "sup") == 0)) {
...
void EV_ClientUserInfoChanged(int cnum) {
    ...
    if (mod_1v1arena->integer) {
        ...
        if (team == 3) {
            Cmd_ExecuteString(
                    va("location %i \"^7[^3Arena ^5SPEC^7][^3Skill ^5%i^7]\" 0 1", client->playernum, client->skill));
            ...
            Cmd_ExecuteString(
                    va("location %i \"^7[^3Arena ^5%i^7][^3Skill ^5%i^7] ^5- ^3facing ^6%s\" 0 1", client->playernum,
                       client->arena, client->skill, cl2->colourName));
        }
    }
    ...
    if (mod_levelsystem->integer) {
        ...
        // Set our name:
        char *mynewname = va("^7[^2%i^7]^3%s^1", client->level,
                             cleanName); //The ^1 at the end will affect the kills/deaths on the mini scoreboard
        ...
        // Shoot a location update for the new name
        Cmd_ExecuteString(
                va("location %i \"%s ^3Level:^7[^2%i^7/^2999^7] ^4- ^3XP:^7[^2%i^7/^25000^7] ^4- ^3Spree: ^2%i\" 0 1",
...
void EV_ClientKill(int cnum, int target) {
    ...
    if (mod_levelsystem->integer) {
        ...
        Cmd_ExecuteString(
                va("location %i \"%s ^3Level:^7[^2%i^7/^2999^7] ^4- ^3XP:^7[^2%i^7/^25000^7] ^4- ^3Spree: ^2%i\" 0 1",
                   cnum, kld->name, kld->level, kld->experience, kld->kills));

        if ((klr->experience + 200) >= 5000) {
            klr->experience = 0;
            klr->level = (klr->level + 1);
            Cmd_ExecuteString(
                    va("bigtext \"%s ^3has just ^2Leveled UP! ^3Level:^7[^2%i^3/^5999^7]\"", klr->name, klr->level));
            ...
            Q_CleanStr(cleanName);
            // Set our name:
            char *mynewname = va("^7[^2%i^7]^3%s^1", klr->level,
                                 cleanName); //The ^1 at the end will affect the kills/deaths on the mini scoreboard
            // Set the new custom name
